...
def define_generator(gen_input_size):
    ...
    gen = BatchNormalization()(gen)
    # upsample to 14x14
    gen = Conv2DTranspose(64, (4, 4), strides=(
        2, 2), padding='same', kernel_initializer=init)(gen)
    gen = Activation('relu')(gen)
    gen = BatchNormalization()(gen)
    # upsample to 28x28
    gen = Conv2DTranspose(1, (4, 4), strides=(
        2, 2), padding='same', kernel_initializer=init)(gen)
    # tanh output
...
def summarize_performance(step, g_model, gan_model, latent_dim, n_cat, n_samples=100):
    if not hasattr(summarize_performance, "imagePlotProcess"):
        summarize_performance.imagePlotProcess = None
...
def load_model(gan_model):
    ...
    import shutil
    gan_model_files = {}
    gan_model_pattern = re.compile(r"gan_model_(\d+)\.tf")
    for root, dirs, files in os.walk('.'):
        for folder in dirs:
            findresult = re.findall(gan_model_pattern, folder)
            if len(findresult) != 1:
                continue
            trainCnt = int(findresult[0])
            gan_model_files[trainCnt] = os.path.join(root, folder)
    if len(gan_model_files.values()) == 0:
        return 0
    latestSave = max(gan_model_files.keys())
    gan_model = tf.keras.models.load_model(gan_model_files[latestSave])
...
def train(g_model, d_model, gan_model, dataset, latent_dim, n_cat, n_epochs=100, n_batch=64):
    ...
    for i in range(n_steps):
        ...
        X_fake, y_fake = generate_fake_samples(
            g_model, latent_dim, n_cat, half_batch)
